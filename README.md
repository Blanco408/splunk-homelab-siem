...
# | SSH Brute Force | Metasploit ssh\_login | /var/log/auth.log | >5 failed logins/min from single IP |
...
# | stats count as failed\_attempts by src\_ip, \_time
...
# | rex "SRC=(?P<src\_ip>\\d+\\.\\d+\\.\\d+\\.\\d+)"

# | bucket \_time span=1m

# | stats count as blocked\_connections by src\_ip, \_time

# | where blocked\_connections > 15
...
# See the `incident\_report\_\*.md` files for sample AI-generated reports.
...
# 3\. Add your Splunk credentials and Anthropic API key to `ai\_triage.py`

# 4\. Run: `python ai\_triage.py`
